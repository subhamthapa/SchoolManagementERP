...
export class LoginService {
  ...
  private getLoginThumbnailObservable() {
    return this.httpClient.get<ThumbnailImage>(
      WebAppConstants.get_login_thumbnail,
  ...
  private logInObservable(username: string, password: string) {
    return this.httpClient.post(WebAppConstants.login, {
      username: username,
      password: password,
    });
  }
  public logIn(username: string, password: string, view: LoginComponent) {
    this.authService.login(username, password).subscribe(
      (success) => {
        this.route.navigate([Utilities.getWebsitebaseUrl() + '/dashboard']);
      },
      (error) => {
        console.log(error);
